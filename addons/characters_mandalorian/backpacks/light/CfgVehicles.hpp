...
class ls_mandalorianBackpack_medic: ls_mandalorianBackpack_light {
    author = AUTHOR;
    displayName = CSTRING(medic_daypack);
    hiddenSelectionsTextures[] = {
        QPATHTOF(backpacks\light\data\camo1_medic_co.paa),
        QPATHTOF(backpacks\light\data\camo1_medic_co.paa)
    };
    picture = QPATHTOF(data\ui\lightBackpack_medic_ui_ca.paa);
};

class ls_mandalorianBackpack_radio: ls_mandalorianBackpack_light {
    author = AUTHOR;
    displayName = CSTRING(radio_daypack);
    hiddenSelectionsTextures[] = {
        QPATHTOF(backpacks\light\data\camo1_co.paa),
        "",
        QPATHTOF(backpacks\light\data\camo1_co.paa)
    };
    picture = QPATHTOF(data\ui\lightBackpack_radio_ui_ca.paa);
...
// Deprecated
DEPRECATED(ls_mandalorian_light_backpack,ls_mandalorianBackpack_light);
DEPRECATED(ls_mandalorian_medic_backpack,ls_mandalorianBackpack_medic);
DEPRECATED(ls_mandalorian_rto_backpack,ls_mandalorianBackpack_radio);
