...
class ls_mandalorianBackpack_demo: ls_mandalorianBackpack {
    author = AUTHOR;
    displayName = CSTRING(demo_backpack);
    maximumload = 320;
    hiddenSelectionsTextures[] = {
        QPATHTOF(backpacks\standard\data\camo1_co.paa),
        "",
    ...
    picture = QPATHTOF(data\ui\standardBackpack_demo_ui_ca.paa);
};
...
class ls_mandalorianBackpack_heavy: ls_mandalorianBackpack {
    ...
    picture = QPATHTOF(data\ui\standardBackpack_heavy_ui_ca.paa);
};
...
class ls_mandalorianBackpack_demo_predef_demo: ls_mandalorianBackpack_demo {
    scope = 1;
    class TransportMagazines {
        MAG_XX(ls_magazine_40mm_3Rnd_IM41_HE,10);
...
DEPRECATED(ls_mandalorian_standard_backpack,ls_mandalorianBackpack);
DEPRECATED(ls_mandalorian_demo_backpack,ls_mandalorianBackpack_demo);
