#define ATRT_EFFECTS_EVENT_ID(var1) (format [QGVAR(atrt_effects_%1), hashValue var1])
...
#define MAX_RPM 6900
#define ENGINE_RPM(rpm) (rpm/MAX_RPM)

// specialized FACTOR for rpm
#define FACTOR_RPM(from,to) WARP_FACTOR(ENGINE_RPM(rpm),ENGINE_RPM(from),ENGINE_RPM(to))
...
// Vehicles with passenger seats will need to manually add `showNVGCargo[] = {...};`
#define PILOT_CARGO_SPEC(SHOW_PILOT_BACKPACK) \
    showNVGDriver = 1; \
    showNVGGunner = 1; \
    showNVGCommander = 1; \
    class PilotSpec { \
        showBackpack = SHOW_PILOT_BACKPACK; \
    }; \
    class CargoSpec { \
        class Cargo1 { \
            showBackpack = 1; \
...
#define TURRET_SPEC(SHOW_BACKPACK) \
    forceNVG = 1; \
    showHMD = 1; \
    showNVGGunner = 1; \
    class TurretSpec { \
