...
#define QUADS(var1,var2,var3,var4) TRIPLES(var1,var2,DOUBLES(var3,var4))
...
#define GET_NAME(var1) (var1 getVariable ["ace_nameRaw", name var1])
...
#define DOOR_POSITION(DOOR_INDEX) QUOTE(TRIPLES(door,DOOR_INDEX,trigger))
#define DOOR_ACTIONS(DOOR_INDEX) \
    class DOUBLES(OpenDoor,DOOR_INDEX) { \
        displayName = "$STR_DN_OUT_O_DOOR"; \
        displayNameDefault = "<img image='\A3\Ui_f\data\IGUI\Cfg\Actions\open_door_ca.paa' size='2.5' />"; \
        textToolTip = "$STR_DN_OUT_O_DOOR"; \
        position = DOOR_POSITION(DOOR_INDEX); \
        radius = 4; \
        aiMaxRange = 5.25; \
        priority = 100; \
        onlyForPlayer = 0; \
        condition = QUOTE([ARR_3(this,DOOR_INDEX,true)] call EFUNC(common,doorCondition)); \
        statement = QUOTE([ARR_3(this,DOOR_INDEX,1)] call EFUNC(common,doorStatement)); \
    }; \
    class DOUBLES(CloseDoor,DOOR_INDEX): DOUBLES(OpenDoor,DOOR_INDEX) { \
        displayName = "$STR_DN_OUT_C_DOOR"; \
        textToolTip = "$STR_DN_OUT_C_DOOR"; \
        condition = QUOTE([ARR_3(this,DOOR_INDEX,false)] call EFUNC(common,doorCondition)); \
        statement = QUOTE([ARR_3(this,DOOR_INDEX,0)] call EFUNC(common,doorStatement)); \
    }
