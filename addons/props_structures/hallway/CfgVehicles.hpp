class ls_prop_hallway_2way: ls_house_base {
    scope = 2;
    scopeCurator = 2;
    author = AUTHOR;
...
class ls_prop_hallway_door_locked: ls_prop_hallway_2way {
    author = AUTHOR;
    displayName = "Hallway Door (Decor)";

    model = QPATHTOF(hallway\ls_prop_hallway_door.p3d);
    hiddenSelections[] = {"door_1a", "door_1b", "frame"};
    hiddenSelectionsTextures[] = {
    ...
    // Define anim sources here so modders can still manually animate them
    numberOfDoors = 0;
...
class ls_prop_hallway_door: ls_prop_hallway_door_locked {
    author = AUTHOR;
