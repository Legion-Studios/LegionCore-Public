...
if !(getText (_configOf >> QEGVAR(dispenser,group)) == "" && { getArray (_configOf >> QEGVAR(dispenser,group)) isEqualTo [] }) exitWith {
    ...
    [QEGVAR(common,addCuratorEditableObjects), _logic] call CBA_fnc_serverEvent;
...
private _fnc_onConfirm = {
    ...
    if (_group select 1 in ["", "[]"] && _groupCustom == "[]") exitWith {
        [objNull, LLSTRING(dispenserGroup_noGroup)] call BIS_fnc_showCuratorFeedbackMessage;
    };

    GVAR(dispenserLastOptions) = [_group, _groupCustom, _spawnLimit];
    _logic setVariable [QGVAR(dispenserGroup), _group, true];
    _logic setVariable [QGVAR(dispenserGroupCustom), _groupCustom, true];
    _logic setVariable [QGVAR(spawnLimit), _spawnLimit, true];
