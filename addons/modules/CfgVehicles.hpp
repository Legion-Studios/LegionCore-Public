// See https://community.bistudio.com/wiki/Modules

class CfgVehicles {
    class Logic;
    class Module_F: Logic {
        class AttributesBase {
            class Default;
            class Edit;
            class Combo;
            class GVAR(Tree): Default {
                displayName = "";
                tooltip = "";
                property = "";
                control = QGVAR(tree);
                defaultValue = QTREE_DEFAULT_VALUE;
            };
            class ModuleDescription;
        };
        class ModuleDescription;
    };
    class MODULE(Base): Module_F {
        scope = 0;
        scopeCurator = 2;
        author = AUTHOR;

        category = QUOTE(ADDON);
        function = QEFUNC(common,dummy);
        functionPriority = 1;
        isGlobal = SERVER_ONLY;
        isTriggerActivated = 0;
    };

    class MODULE(ToggleCamo): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleToggleCamo_displayName);
        icon = QPATHTOF(data\ui\moduleToggleCamo_ui_ca.paa);

        isGlobal = GLOBAL;
        curatorCanAttach = 1;
        function = QFUNC(moduleToggleCamo);
    };

    class MODULE(DeleteGroups): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleDeleteGroups_displayName);
        function = QFUNC(moduleDeleteGroups);
        isGlobal = GLOBAL;
    };

    class MODULE(AddReinsertAction): MODULE(Base) {
        scope = 2;
        scopeCurator = 0;
        author = AUTHOR;
        displayName = CSTRING(moduleAddReinsertAction_displayName);
        function = QFUNC(moduleAddReinsertAction);
        isGlobal = GLOBAL;

        class Attributes: AttributesBase {
            class ModuleDescription: ModuleDescription {};
        };

        class ModuleDescription: ModuleDescription {
            description = CSTRING(moduleAddReinsertAction_description);
        };
    };

    class MODULE(AddReinsertAction_zeus): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleAddReinsertAction_displayName);
        function = QFUNC(moduleAddReinsertAction);
        isGlobal = GLOBAL;
        curatorCanAttach = 1;
    };

    class MODULE(AreaHeal): MODULE(Base) {
        scope = 2;
        scopeCurator = 0;
        displayName = CSTRING(moduleAreaHeal_displayName);
        icon = QPATHTOEF(data,icons\medic_white_ca.paa);

        function = QFUNC(moduleAreaHeal);

        class Attributes: AttributesBase {
            class ModuleDescription: ModuleDescription {};
        };

        canSetArea = 1;
        canSetAreaShape = 1;
        canSetAreaHeight = 1;
        class AttributeValues {
            size3[] = {30, 30, 15};
            isRectangle = 1;
        };

        class ModuleDescription: ModuleDescription {
            description = CSTRING(moduleAreaHeal_description);
        };
    };

    class MODULE(BreachDoor): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleBreachDoor_displayName);
        icon = QPATHTOF(data\ui\moduleBreachDoor_ui_ca.paa);
        portrait = QPATHTOF(data\ui\moduleBreachDoor_ui_ca.paa);
        curatorInfoType = QGVAR(RscBreachDoor);
        curatorCanAttach = 1;
    };

    class MODULE(MountATRT): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleMountATRT_displayName);
        // icon = QPATHTOF(data\ui\moduleMountATRT_ui_ca.paa);
        // portrait = QPATHTOF(data\ui\moduleMountATRT_ui_ca.paa);
        curatorInfoType = QGVAR(RscMountATRT);
        curatorCanAttach = 1;
    };

    class MODULE(DismountATRT): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleDismountATRT_displayName);
        // icon = QPATHTOF(data\ui\moduleDismountATRT_ui_ca.paa);
        // portrait = QPATHTOF(data\ui\moduleDismountATRT_ui_ca.paa);
        isGlobal = GLOBAL;
        curatorCanAttach = 1;
        function = QFUNC(moduleDismountATRT);
    };

    class MODULE(SetSkills): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = CSTRING(moduleSetSkills_displayName);
        curatorInfoType = QGVAR(RscSetSkills);
        curatorCanAttach = 1;
    };

    class MODULE(AircraftSpawner): MODULE(Base) {
        scope = 2;
        scopeCurator = 0;
        author = AUTHOR;
        displayName = CSTRING(moduleAircraftSpawner_displayName);

        icon = "\A3\data_f_jets\logos\jets_logo_small_ca.paa";
        portrait = "\A3\data_f_jets\logos\jets_logo_small_ca.paa";

        function = QFUNC(moduleVehicleSpawner);
        isTriggerActivated = 1;
        isGlobal = SERVER_ONLY;

        class Attributes: AttributesBase {
            class GVAR(spawnTrigger): Combo {
                displayName = CSTRING(spawnTrigger_label);
                tooltip = CSTRING(spawnTrigger_tooltip);
                property = QGVAR(spawnTrigger);
                class values {
                    class WaveDestroyed {
                        name = CSTRING(spawnTrigger_waveDestroyed_label);
                        tooltip = CSTRING(spawnTrigger_waveDestroyed_tooltip);
                        defaultValue = "WaveDestroyed";
                        value = "WaveDestroyed";
                        default = 1;
                    };
                    class Continious {
                        name = CSTRING(spawnTrigger_continious_label);
                        tooltip = CSTRING(spawnTrigger_continious_tooltip);
                        defaultValue = "Continious";
                        value = "Continious";
                    };
                };
            };
            class GVAR(spawnDelay): Default {
                displayName = CSTRING(spawnDelay_label);
                tooltip = CSTRING(spawnDelay_tooltip);
                property = QGVAR(spawnDelay);
                defaultValue = QUOTE(SPAWN_DELAY_DEFAULT); // Time in seconds
                control = "SliderTime";
            };
            class GVAR(waveSize): Edit {
                displayName = CSTRING(waveSize_label);
                tooltip = CSTRING(waveSize_tooltip);
                property = QGVAR(waveSize);
                defaultValue = QUOTE(WAVE_SIZE_DEFAULT);
                validate = "NUMBER";
            };
            class GVAR(vehicleTypes): GVAR(Tree) {
                displayName = CSTRING(vehicleTypes_label);
                tooltip = CSTRING(vehicleTypes_tooltip);
                property = QGVAR(vehicleTypes);
                control = QGVAR(airVehicleTypes);
                defaultValue = QVEHICLE_TYPES_DEFAULT;
            };
            class GVAR(executeCode): Default {
                displayName = CSTRING(executeCode_label);
                tooltip = CSTRING(executeCode_waveSpawn_tooltip);
                property = QGVAR(executeCode);
                control = "EditCodeMulti3";
                validate = "string";
                defaultValue = "''";
            };
            class ModuleDescription: ModuleDescription {};
        };

        class ModuleDescription: ModuleDescription {
            position = 1;
            direction = 1;
            description = CSTRING(moduleAircraftSpawner_description);
        };
    };

    class MODULE(AircraftSpawner_zeus): MODULE(Base) {
        scope = 1;
        scopeCurator = 2;
        author = AUTHOR;
        displayName = CSTRING(moduleAircraftSpawner_displayName);

        icon = "\A3\data_f_jets\logos\jets_logo_small_ca.paa";
        portrait = "\A3\data_f_jets\logos\jets_logo_small_ca.paa";

        curatorInfoType = QGVAR(RscAircraftSpawner);
        curatorCanAttach = 1;
    };

    // TODO: Future potential modules, vehicle spawner for players where mission makers can select vehicles from tree view
    //       Also "landcraft spawner", aircraft spawner but for like tanks and cars

    class MODULE(DroidDispenser): MODULE(Base) {
        scope = 2;
        scopeCurator = 0;
        author = AUTHOR;
        displayName = ECSTRING(dispenser,displayName);

        // simulation = "house"; // Changing simulation breaks the isTriggerActivated condition
        model = "\a3\Modules_F_Curator\Ordnance\surfaceMortar.p3d";
        icon = QPATHTOEF(dispenser,data\ui\droidDispenser_icon_ca.paa);
        portrait = QPATHTOEF(dispenser,data\ui\droidDispenser_icon_ca.paa);

        function = QFUNC(moduleDroidDispenser);
        isTriggerActivated = 1;
        ammo = QEGVAR(dispenser,ordnance);

        // Can be name in ls_dispenser_groups (e.g. ls_cis-cis_baseInfantry-base_b1_fireteam) or array of unit class names
        // Default ("" or []) means open dispenser menu to select a group
        EGVAR(dispenser,group) = "";
        EGVAR(dispenser,vehicle) = "ls_droidDispenser"; // Vehicle to use, must be child class of ls_droidDispenser

        class Attributes: AttributesBase {
            class GVAR(dispenserGroup): GVAR(Tree) {
                displayName = CSTRING(dispenserGroup_group_label);
                tooltip = CSTRING(dispenserGroup_group_tooltip);
                property = QGVAR(dispenserGroup);
                control = QGVAR(dispenserGroup);
                defaultValue = QDISPENSER_GROUP_DEFAULT;
            };
            class GVAR(dispenserGroupCustom): Edit {
                displayName = CSTRING(dispenserGroup_customGroup_label);
                tooltip = CSTRING(dispenserGroup_customGroup_tooltip);
                property = QGVAR(dispenserGroupCustom);
                defaultValue = "'[]'";
            };
            class GVAR(spawnLimit): Edit {
                displayName = CSTRING(spawnLimit_label);
                tooltip = CSTRING(spawnLimit_tooltip);
                property = QGVAR(spawnLimit);
                defaultValue = SPAWN_LIMIT_DEFAULT;
                validate = "NUMBER";
            };
            class ModuleDescription: ModuleDescription {};
        };

        class ModuleDescription: ModuleDescription {
            position = 1;
            description = CSTRING(moduleDroidDispenser_description);
        };
    };

    class MODULE(DroidDispenser_zeus): MODULE(Base) {
        scope = 1;
        author = AUTHOR;
        displayName = ECSTRING(dispenser,displayName);

        simulation = "house";
        model = "\A3\Modules_F_Curator\Ordnance\surfaceMortar.p3d";
        icon = QPATHTOEF(dispenser,data\ui\droidDispenser_icon_ca.paa);
        portrait = QPATHTOEF(dispenser,data\ui\droidDispenser_icon_ca.paa);

        ammo = QEGVAR(dispenser,ordnance);
        curatorInfoType = QGVAR(RscDroidDispenser);
        isGlobal = GLOBAL; // Used to hide module's "ring" for non-zeus clients

        EGVAR(dispenser,vehicle) = "ls_droidDispenser";
        EGVAR(dispenser,group) = ""; // Group from ls_dispenser_groups or array of unit classes. "" = open menu
    };
    class MODULE(DroidDispenser_B1_zeus): MODULE(DroidDispenser_zeus) {
        author = AUTHOR;
        displayName = ECSTRING(dispenser,B1_displayName);
        EGVAR(dispenser,group) = "ls_cis>>cis_baseInfantry>>base_b1_fireteam";
    };
    class MODULE(DroidDispenser_B2_zeus): MODULE(DroidDispenser_zeus) {
        author = AUTHOR;
        displayName = ECSTRING(dispenser,B2_displayName);
        EGVAR(dispenser,group)[] = {"ls_droid_b2"};
        EGVAR(dispenser,limit) = 10;
    };
    class MODULE(DroidDispenser_BX_zeus): MODULE(DroidDispenser_zeus) {
        author = AUTHOR;
        displayName = ECSTRING(dispenser,BX_displayName);
        EGVAR(dispenser,group) = "ls_cis>>cis_specialOperations>>specops_bx_fireteam";
        EGVAR(dispenser,limit) = 10;
    };

    // Eventually will be moved to battlefields
    class lsb_moduleSpaceDebris: MODULE(Base) {
        scope = 2;
        scopeCurator = 0;
        author = AUTHOR;
        displayName = "$STR_lsb_modules_spaceDebris_name";

        icon = "lsb_iconModuleSpaceDebris";

        is3DEN = 1;
        function = QFUNC(moduleSpaceDebris);

        class Attributes: AttributesBase {
            // People may want to pull these values, so write lsb
            class lsb_modules_count: Edit {
                displayName = "$STR_lsb_modules_AttributeCount_name";
                tooltip = "$STR_lsb_modules_spaceDebris_AttributeCount_tooltip";
                property = "lsb_modules_count";
                defaultValue = 300;
                validate = "NUMBER";
            };

            class lsb_modules_preset: Combo {
                displayName = "$STR_lsb_common_spaceDebrisPreset_name";
                tooltip = "$STR_lsb_common_spaceDebrisPreset_tooltip";
                property = "lsb_modules_preset";
                control = "lsb_modules_spaceDebrisPreset";
                defaultValue = "''";
            };

            class lsb_modules_distribution: Combo {
                displayName = "$STR_lsb_common_distribution_name";
                tooltip = "$STR_lsb_common_distribution_tooltip";
                property = "lsb_modules_distribution";
                control = "lsb_modules_distribution";
                defaultValue = "'randomArea'";
            };

            // class lsb_modules_drifting: Checkbox {
            //     displayName = "$STR_lsb_modules_spaceDebris_AttributeDrifting_name";
            //     tooltip = "$STR_lsb_modules_spaceDebris_AttributeDrifting_tooltip";
            //     property = "lsb_modules_drifting";
            //     defaultValue = "false";
            // };

            // class lsb_modules_driftDirection: Edit {
            //     displayName = "$STR_lsb_modules_spaceDebris_AttributeDriftDirection_name";
            //     tooltip = "$STR_lsb_modules_spaceDebris_AttributeDriftDirection_tooltip";
            //     property = "lsb_modules_driftDirection";
            //     defaultValue = "0";
            //     validate = "NUMBER";
            //     control = "EditZ";
            // };

            class lsb_modules_debrisData: Edit {
                displayName = "$STR_lsb_modules_spaceDebris_AttributeDebrisData_name";
                tooltip = "$STR_lsb_modules_spaceDebris_AttributeDebrisData_tooltip";
                property = "lsb_modules_debrisData";
                defaultValue = "''";
            };

            class lsb_modules_paramsOld: Edit {
                displayName = "$STR_lsb_modules_spaceDebris_AttributeParamsOld_name";
                tooltip = "$STR_lsb_modules_spaceDebris_AttributeDebrisData_tooltip";
                property = "lsb_modules_paramsOld";
                defaultValue = "'[]'";
            };
        };

        canSetArea = 1;
        canSetAreaShape = 1;
        canSetAreaHeight = 1;
        class AttributeValues {
            size3[] = {500, 500, 250};
        };

        class ModuleDescription: ModuleDescription {
            direction = 1;
            description = "$STR_lsb_modules_spaceDebris_description";
        };
    };

    class ModuleOrdnance_F;
    #include "cfg\protonMortar\CfgVehicles.hpp"
};
