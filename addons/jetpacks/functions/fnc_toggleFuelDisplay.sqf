 ...
 *
 * Example:
 * call ls_jetpacks_fnc_toggleFuelDisplay;
 *
 * Public: No
 */

TRACE_1("fnc_toggleFuelDisplay",ls_player);
...
if !(alive ls_player and
    {call CBA_fnc_getActiveFeatureCamera == ""} and
    {ls_player call _fnc_isNotCrew} and
    {ls_player call FUNC(hasJetpack)}
) exitWith {
...
ls_player call FUNC(updateFuelDisplay);
