...
private _index = switch (_weapon) do {
    case (secondaryWeapon _unit): { 1 };
    case (handgunWeapon _unit): { 2 };
    default { 0 };
};
(_unit weaponState _weapon) params ["", "_muzzle", "_fireMode"];
...
// Re-select weapon, need to get new weapon name since it might have changed
_weapon = _loadout select _index select 0;
