class CfgWeapons {
    ...
    class ls_cloneUniform_base: ls_uniform_base {
        // Vanilla uniforms don't have armor level descriptions, but our clone uniforms have the same armor levels as a uniform + vest
        // Makes it easier to tell for non-ACE players to see that they have armor
    ...
    class ls_cloneVest_basic: ls_cloneVest_base {
        scope = 2;
        author = AUTHOR;
        displayName = CSTRING(basic_vest);

        model = QPATHTOF(vests\basic\ls_vest_clone.p3d);
        hiddenSelections[] = {};
        hiddenSelectionsTextures[] = {};

        class ItemInfo: ItemInfo {
            uniformModel = QPATHTOF(vests\basic\ls_vest_clone.p3d);
            hiddenSelections[] = {};
        };
    };
