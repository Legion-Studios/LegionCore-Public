class ls_westarM5_Shot_SoundSet {
    soundShaders[] = {"ls_westarM5_closure_SoundShader", "ls_westarM5_closeShot_SoundShader", "ls_westarM5_midShot_SoundShader", "ls_westarM5_distShot_SoundShader"};
    volumeFactor = 1;
    frequencyRandomizer = 0.75;
    frequencyRandomizerMin = 0.25;
    volumeCurve = "ls_weapons_basic_volumeCurve";
    sound3DProcessingType = "ls_weapons_3dProcessor";
    distanceFilter = "ls_weapons_lowpass_filter";
    occlusionFactor = 0.25;
    obstructionFactor = 0.3;
    spatial = 1;
    doppler = 0;
    loop = 0;
};