 ...
 *
 * Example:
 * [bob, "DefaultAction", "condition", "execute"] call ls_common_fnc_addActionEventHandler
 *
 * Public: No
 */

params ["_unit", "_action", "_condition", "_statement"];
TRACE_4("fnc_addActionEventHandler",_unit,_action,_condition,_statement);

private _name = format ["ls_action_%1", _action];
...
if (_actionID == -1) then {
    private _addAction = call compile format [
        "[
            ...
            true,
            '%1',
            ""if (_this != ls_player) exitWith {false}; _actions = (_this getVariable '%2') select 1 select 2; _count = count _actions; _index = 0; _return = false; while {_index < _count && {!_return}} do {_return = [_target, _this] call ((_actions select _index) select 0); _index = _index + 1}; _return""
        ]",
        _action,
        _name
    ];
