class ls_staticShip_coreShip: ls_staticShip_multiPart_base {
    ...
    EGVAR(map,icon) = QPATHTOF(data\ui\map\coreship_icon_ca.paa);
    EGVAR(map,size) = 454.95;
...
class ls_staticShip_coreShip_zeus: ls_staticShip_base {
    scope = 2;
    scopeCurator = 2;
    author = AUTHOR;
    displayName = "Core Ship (Zeus)";
    model = QPATHTOF(coreship\ls_coreShip_zeus.p3d);

    EGVAR(map,icon) = QPATHTOF(data\ui\map\coreship_icon_ca.paa);
    EGVAR(map,size) = 454.95;
};

class ls_staticShip_coreShip_platform_zeus: ls_staticShip_base {
    scope = 2;
    scopeCurator = 2;
    author = AUTHOR;
    displayName = "Core Ship Platform (Zeus)";
    model = QPATHTOF(coreship\ls_coreShipPlatform_zeus.p3d);
};
